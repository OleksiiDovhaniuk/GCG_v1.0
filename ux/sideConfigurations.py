...
from ux.cell import AddCell, Cell, EmptyCell, IndexCell, SwitchCellBtn, TitleCell
from ux.dialog import Load, Save, TruthTable
from ux.layout import GreyDefault, LayoutConf, TTblRow, Separator10, Line1Dark
from ux.lbl import InfoCoefLbl, InfoLbl, Lbl, ResultsLbl, TitleLbl
from ux.popup import WhitePopup
from ux.radioButton import RbtEndCondition
from ux.textInput import AlgorithmCoefInput, AlgorithmConfigsInput, DeviceNameInput
from ux.widgets import HistoryWidget
from design import Design


Builder.load_file('ui/sideConfigurations.kv')
...
class Algorithm(SideConf):
    ...
    def __init__(self):
        ...
        for key in configs:
            ...
            if key == 'Control Gates` Number':
                ...
                for name, dvalue in zip(coefs, data[0]):
                    self.draw_config(
                        name, dvalue, *data[1:4], intent=1, is_coef=True)
        ...
        widgets['Process Time']['Radio Button'].mates = [
            widgets['Iterations Limit']]
        widgets['Iterations Limit']['Radio Button'].mates = [
            widgets['Process Time']]
    ...
    def bend_related_extremes(self):
        ...
        for inpt in txt_inputs:
            if type(inpt.valid_range[0]) == str:
                inpt.related_min = self.widgets[inpt.valid_range[0]
                                                ]['Text Input']

            if type(inpt.valid_range[1]) == str:
                inpt.related_max = self.widgets[inpt.valid_range[1]
                                                ]['Text Input']
        ...
        for key in configs:
            if key == 'Control Gates` Number':
                for index, key2 in enumerate(('min', 'max')):
                    ...
                    widgets[key2]['Text Input'].hint_text = str(
                        configs[key]['value'][index])
                ...
                widgets[key]['Text Input'].hint_text = str(
                    configs[key]['value'])
                widgets[key]['Radio Button'].set_status(
                    configs[key]['is active'])
...
class Input(SideConf):
    ...
    def draw_truth_table(self):
        ...
        for index, signal in enumerate(data['inputs']):
            ...
            title_cell = TitleCell(
                ...
                save_switch=lambda title, sign_type: self.save_switch(
                    title, sign_type),
            )
    ...
    def get_values(self):
        ...
        inputs = [[[key, int(signal)] for signal in ttbl['inputs'][key]]
                  for key in ttbl['inputs']]
        outputs = [[[key, int(signal)] for signal in ttbl['outputs'][key]]
                   for key in ttbl['outputs']]
        values = inputs
    ...
    def del_row(self, index):
        ...
        for sign_type in ('inputs', 'outputs'):
            for key in ttbl[sign_type]:
                ttbl[sign_type][key] = ttbl[sign_type][key][:index] + \
                    ttbl[sign_type][key][index+1:]
    ...
    def del_column(self, index):
        ...
        widgets['Switches` Layout'].remove_widget(
            widgets['Switches'].pop(index))
        if widgets['Titles'][index].reserved_title in self.device['Truth Table']['inputs'].keys():
            self.device['Truth Table']['inputs'].pop(
                widgets['Titles'][index].reserved_title)
        elif widgets['Titles'][index].reserved_title in self.device['Truth Table']['outputs'].keys():
            self.device['Truth Table']['outputs'].pop(
                widgets['Titles'][index].reserved_title)
    ...
    def save_value(self, title, index, value, *args):
        ...
        if title in inputs:
            inputs[title] = inputs[title][:index] + \
                value + inputs[title][index+1:]
        elif title in outputs:
            outputs[title] = outputs[title][:index] + \
                value + outputs[title][index+1:]
        else:
...
class Plot(SideConf):
    ...
    def __init__(self):
        ...
        threshold = self.data['Algorithm']['configurations']['Fitness Function Coeficients']['value'][0]
        # Max number of generations
        generation_no = self.data['Algorithm']['configurations']['Iterations Limit']['value']
        ...
        box = BoxLayout(pos_hint={'center_x': .5, 'center_y': .5})
        # box.add_widget(fcka(plt.gcf()))
        container.add_widget(box)
        container.add_widget(BoxLayout())


class ResultsContent(ScrollView):
...
class Results(SideConf):
    ...
    def __init__(self, **kwargs):
        ...
        container = self.ids.container
        container.clear_widgets()
        self.content = content = ResultsContent()
