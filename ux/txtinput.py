...
Builder.load_file('ui/txtinput.kv')
...
class AlgorithmConfigsInput(TxtInput):
    ...
    def show_warning(self, dmin=None, dmax=None, dtype=None):
        ...
        if dtype:
            warning += f'non-negative {dtype}-type.'
        elif dmin:
            warning += f'not less than {dmin}.'
        elif dmax:
            warning += f'not bigger than {dmax}.'
        else:
            warning = '[ERROR]: Unknown issue in the code!'
...
class PropertyTxtInput(TxtInput):
    def change(step):
        pass
